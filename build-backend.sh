...
docker run --rm --user node:node -v $(pwd)/backend_src/app:/app -w /app wisecat/chatwheel-backend-build npm install
docker run --rm --user node:node -v $(pwd)/backend_src/app:/app -w /app wisecat/chatwheel-backend-build npm run build
